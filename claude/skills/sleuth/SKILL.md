...
- Unfamiliar with the code path involved? Delegate to the `watson` subagent to trace how it actually works before generating hypotheses — don't guess your way through code you haven't read.
...
- Good: bug is in an unfamiliar auth middleware chain → delegate to `watson` for a trace/diagram of how it currently works, then use that to generate grounded hypotheses instead of guessing.
